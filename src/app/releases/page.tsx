    ...
    <AppShell variant="light">
      <div className="space-y-8">
        ...
        <form className="grid gap-3 rounded-xl border border-stone-200 bg-white p-4 shadow-sm md:grid-cols-[1fr_13rem_11rem_auto]" action="/releases">
          ...
          <select name="status" defaultValue={query?.status ?? "all"} aria-label="Filter by evidence status" className="h-11 rounded-md border border-stone-300 bg-white px-3 text-sm font-bold text-stone-950"><option value="all">All evidence</option><option value="structured">Structured values</option><option value="narrative-report">Narrative reports</option><option value="summary">Summary only</option><option value="source-linked">Source linked</option></select>
          ...
          <div className="mt-5 divide-y divide-stone-200 border-y border-stone-200">
            {releases.slice(0, 80).map((release) => (
              <article key={release.id} className="grid gap-4 py-5 lg:grid-cols-[10rem_1fr_auto] lg:items-start">
                ...
                <div><h3 className="text-xl font-black leading-snug text-stone-950">{release.title}</h3><p className="mt-2 line-clamp-2 text-sm leading-6 text-stone-600">{release.summary}</p><div className="mt-3 flex flex-wrap gap-2"><span className={`rounded-md px-2 py-1 text-xs font-black ${release.status === "structured" ? "bg-emerald-50 text-emerald-800" : release.status === "narrative-report" ? "bg-sky-50 text-sky-900" : release.status === "summary" ? "bg-amber-50 text-amber-900" : "bg-stone-100 text-stone-700"}`}>{release.status === "structured" ? `${release.metricCount} structured values` : release.status.replaceAll("-", " ")}</span>{release.areas.slice(0, 3).map((area) => <span key={area} className="rounded-md bg-stone-100 px-2 py-1 text-xs font-bold text-stone-600">{area}</span>)}</div></div>
