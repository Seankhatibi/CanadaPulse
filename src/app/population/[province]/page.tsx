...
export default async function Page({ params }: { params: Promise<{ province: string }> }) { const { province } = await params; const brief = await getProvinceResearchBrief(province, "population"); if (!brief) notFound(); return <ProvinceResearchPage brief={brief} />; }
