...
export default async function Page({ params }: { params: Promise<{ province: string }> }) { const { province } = await params; const brief = await getProvinceResearchBrief(province, "housing"); if (!brief) notFound(); return <ProvinceResearchPage brief={brief} />; }
