...
function signalFor(chartTitle: string, label: string, direction: ReleaseChartPayload["points"][number]["direction"]) {
  ...
  const context = `${chartTitle} ${label}`;
  const pressureWhenRising = /rent|price|cost|unemployment|debt|deficit|inflation|mortgage|layoff|closure/i.test(context);
  const reliefWhenRising = /vacancy|employment|wage|income|productivity|export|investment|starts|completions|sales/i.test(context);
  ...
  return (
    <article className="border-t border-stone-200 py-6 first:border-0 first:pt-0 last:pb-0">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        <h3 className="text-xl font-black text-stone-950">{chart.title}</h3>
        <p className="text-xs font-semibold text-stone-500">{isMovementChart ? "Change from the comparable period" : "Ranked official values"}</p>
      </div>
    ...
    <section className="rounded-2xl border border-stone-200 bg-white p-5 sm:p-7">
      <div className="max-w-3xl">
        ...
        <h2 className="mt-1 text-3xl font-black text-stone-950">
          {hasMeasuredCharts ? "See what moved underneath" : "What the official report discusses"}
        </h2>
