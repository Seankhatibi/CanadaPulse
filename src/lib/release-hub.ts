  ...
  return {
    ...
    sourceLinks: [
      ...
      ...((releaseData?.wdsDownloads ?? [])
        ...
        .map((download) => ({ label: `WDS ${download.productId}`, url: download.downloadUrl as string }))),
    ],
    importanceScore,
...
async function getBankOfCanadaRelease(): Promise<NormalizedRelease> {
  ...
  return {
    ...
    sourceLinks: [
      ...
      { label: "Bank of Canada Valet API", url: "https://www.bankofcanada.ca/valet/docs" },
    ],
    importanceScore: 86,
    youthImpactScore: 82,
...
async function getCmhcHousingWatch(): Promise<NormalizedRelease> {
  ...
  return {
    ...
    headlineFacts: [
      ...
      "Starts are not completions: Canada Pulse should treat this as a supply pipeline signal, not homes ready to move into.",
    ],
...
async function getSourceLinkedReleases() {
  return [
    sourceLinkedRelease({
      ...
      summary:
        "Energy releases should feed oil, gas, electricity mix, electricity prices, emissions, pipeline and export dashboards.",
      icon: Factory,
      score: 70,
    }),
    sourceLinkedRelease({
      ...
      summary:
        "PBO reports should be used for fiscal outlooks, infrastructure spending, program costing, and debt sustainability reality checks.",
      icon: Building2,
      score: 72,
    }),
  ];
...
export async function getMultiSourceReleaseHub(): Promise<ReleaseHubPayload> {
  ...
  const housingWatch = await getCmhcHousingWatch().catch(() =>
    sourceLinkedRelease({
      ...
      releaseType: "housing-release-monitor",
      affectedAreas: ["housing"],
      summary: "CMHC housing data should feed starts, completions, rent, vacancy, mortgage and debt dashboards.",
      icon: Home,
      score: 95,
    }),
  );
  const bankOfCanada = await getBankOfCanadaRelease().catch(() =>
    sourceLinkedRelease({
      ...
      affectedAreas: ["rates", "housing"],
      summary: "Bank of Canada rate and yield data should feed mortgage, rent, debt and CAD pressure modules.",
      icon: Landmark,
      score: 84,
    }),
  );
  ...
  const ircc = await getOpenGovIrccRelease().catch(() =>
    sourceLinkedRelease({
      ...
      summary: "IRCC open data should feed PR, TFW, student, refugee and asylum dashboards.",
      icon: Users,
      score: 84,
    }),
  );
